...
(if (empty? (cdr fs))
  (cont (car fs))
  (pipe$ (cdr fs) (lambda(res_pipe) (compose$ (car fs) res_pipe cont))))))
...
(lambda (reducer init lst)
  (if (empty? lst)
    init
  (reduce reducer (reducer init (head lst)) (cdr lst)))))

; Signature: reduce-prim$(reducer, init, lst, cont)
; Type: [[T2*T1 -> T2] * T2 * list(T1) * [T2 -> T3] -> T3]
...
(lambda (reducer init lst cont)
  (if (empty? lst)
  (cont init)
  (reduce-prim$ reducer (reducer init (head lst)) (cdr lst) cont))))

; Signature: reduce-user$(reducer, init, lst, cont)
; Type: [[T2*T1*[T2 -> T3] -> T3] * T2 * list(T1) * [T2 -> T4] -> T4]
; Purpose: Returns the reduced value of the given list, from left 
;          to right, with cont post-processing
...
(lambda (reducer$ init lst cont)
  (if (empty? lst)
    (cont init)
    (reducer$ init (head lst) (lambda(res_ducer) (reduce-user$ reducer$ res_ducer (cdr lst) cont))))))
