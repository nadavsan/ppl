...
import { isBoolExp, isCExp, isLitExp, isNumExp, isPrimOp, isStrExp, isVarRef, isSetExp,
         ...
         parseL4Exp, DefineExp, isTraceExp as isTraceExp, TraceExp, makeVarRef, parseL4, parseL4Program, parseL4GoodProgram} from "./L4-ast";
...
const traceMap:Map<string,number> =  new Map();
...
const evalTraceExp = (exp: TraceExp, env: Env): Result<void> =>
    ...
    const val = getFBindingVal(bdg)
       if (isClosure(val)){ 
        traceMap.set(getFBindingVar(bdg), 0)
        return makeOk(setFBinding(bdg, makeTracedClosure(val, getFBindingVar(bdg))))
    }
...
const applyTracedClosure = (proc: TracedClosure, args: Value[]): Result<Value> => {
    ...
    const num = traceMap.get(proc.name);
    if(isNumber(num)){
        printPreTrace(proc.name, args, num);
        traceMap.set(proc.name, num + 1)
    }
    return bind(evalSequence(proc.closure. body, makeExtEnv(vars, args, proc.closure.env)), 
        (v: Value ) =>
        { 
            const num = traceMap.get(proc.name);
            if(isNumber(num)){
                printPostTrace(v, num - 1);
                traceMap.set(proc.name, num - 1)}
            return makeOk(v);}
            )
}
