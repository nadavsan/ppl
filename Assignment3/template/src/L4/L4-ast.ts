...
export const parseTraceExp: (params: Sexp[]) => Result<TraceExp> = 
    ...
    ! isIdentifier(params[0]) ? makeFailure("First arg of trace must be an identifier") :
...
const x = parseL4("(L4 (lambda (x y) (+ x y) (if (x>y) x y)))");
if(isOk(x)){
    if(isProgram(x.value)){
        if(isProcExp(x.value.exps[0])){
            console.log(x.value.exps[0].body[0]); 
        }
    }
}
