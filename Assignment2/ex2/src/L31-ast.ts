...
export interface LetPlusExp {tag: "LetPlusExp"; bindings:Binding[], body:CExp[];} //added
...
export const makeLetPlusExp = (bindings: Binding[], body: CExp[]): LetPlusExp => 
    ({tag: "LetPlusExp", bindings: bindings, body: body}); //added
...
export const isLetPlusExp = (x:any): x is LetPlusExp => x.tag === "LetPlusExp"; //added
...
export const parseL31SpecialForm = (op: Sexp, params: Sexp[]): Result<CExp> =>
    ...
    op === "let*" ? parseLetPlusExp(first(params), rest(params)) : //added
    makeFailure("Never");
...
const parseLetPlusExp = (bindings: Sexp, body: Sexp[]): Result<LetPlusExp> => { //added
    if (!isGoodBindings(bindings)) {
        return makeFailure('Malformed bindings in "let*" expression');
    }
    // Given (letrec ( (var <val>) ...) <cexp> ...)
...
export const unparseL31 = (exp: Program | Exp): string =>
    ...
    isLetPlusExp(exp) ? unparseLetPlusExp(exp): //added
    exp;
