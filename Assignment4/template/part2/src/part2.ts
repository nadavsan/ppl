...
export function makeTableService<T>(sync: (table?: Table<T>) => Promise<Table<T>>): TableService<T> {
    ...
    return {
        get(key: string): Promise<T> {
            ...
            sync()
                .then((table) => ((table[key] != undefined) ? resolve(table[key]) : reject(MISSING_KEY)))
                .catch((err) => (console.error('falied', err)))
            })
        },
        set(key: string, val: T): Promise<void> {
            return new Promise (function (resolve,reject){
                sync() 
                    .then((table) => {
                        let new_table: Record<string, Readonly<T>> = table;
                        if(table[key] != undefined){
                            new_table[key] = val;
                        }
                                    return sync(new_table)
                                        .then((table)=>resolve())
                                        .catch((err) => (console.error('falied', err)))
                                })
        })},
